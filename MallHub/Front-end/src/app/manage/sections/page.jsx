...
const AdminSectionsPage = () => {
    ...
    useEffect(() => {
        const init = async () => {
            const request = await AdminService.getPaginatedSections({ ...pagination });
            setData(request);
        }
        init().then();
    }, [pagination]);
    ...
    return (<div className="space-y-2">
        <div className="flex-between mb-4">
            <div className="flex items-center gap-3">
                <h1 className="h2-bold">Sections</h1>
            </div>
            <div className="flex gap-x-2">
                <Button asChild variant={`default`}>
                    <Link to={`/manage/sections/create`}>
                        Create Section
                    </Link>
                </Button>
            </div>
        ...
        <div className="rounded-md border">
            <Table>
                ...
                <TableBody>
                    {data?.sections?.map((category) => (
                        <TableRow key={category.id}>
                            <TableCell>{category.name}</TableCell>
                            <TableCell className="flex gap-1">
                                <Button asChild variant={`outline`}>
                                    <Link to={`/manage/sections/${category.id}`}>
                                        Edit
                                    </Link>
                                </Button>
